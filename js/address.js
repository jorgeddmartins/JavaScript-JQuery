...
$(document).on('click', '.address_finder_error', function() {
	...
	$('.address_finder_dropdown').hide();
	var $validation_display = $('#address_finder').closest('.validation-display');
	$validation_display
		.removeClass('success')
	...
	$validation_display.find('fieldset').append('<textarea id="address" name="address" placeholder="Address*" autocomplete="off"></textarea>');
	$('input[name=address]').remove();
	$('label[for=address]').css('height', '68px');
});
