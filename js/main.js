...
function process_additional_validation(callback) {
	...
	if ( parseInt($repay_per_month_field.val().replace(/[^0-9]/g, '')) < 75 ) {
		$repay_per_month_field.val('Â£75');
	}
...
$(document).on('click', '.not-more-than-5k', function(e) {
	e.preventDefault();
	alert('Sorry, you must have a minimum of Â£5,000 in debt to apply.');
});
...
$(document).keyup(function(e) {
	...
	if(code == 13 || code == 10) {
		if ( ! $('textarea[name="address"]').is(':visible') ) {
...
console.log('checking data8');
			$('#form_submit_button').html('<img src="/images/pfl-loading.gif"> Please Wait&hellip;').addClass('please-wait');
			check_data8(phone_input, mobile_input, function(data8_result) {
				if ( data8_result['success'] ) {
					...
					*/
					...
					$('#form_submit_button').html('Get a Free Quote').removeClass('please-wait');
	...
	console.log('Step: ' + step);
			...
			$('.container .btn-c, .container .safe-text').css('right', -1000).css('visibility','');
			$form.removeClass("active");
			$form.next().addClass('active').animate({ right : 0 }, 250, function() { $form_container.css('overflow', 'visible'); } );
			if ( ( step < 6 && ! include_address ) || ( step < 7 && include_address ) ) {
				$('.container .btn-c, .container .safe-text').animate({ right : 0 }, 250, function() { $form_button_container.css('overflow', 'visible'); } );
			}
			else {
				$form.next().find('.btn-c, .safe-text').animate({ right : 0 }, 200, function() { $form_button_container.css('overflow', 'visible'); } );
			}
...
$(document).on('click', '.show-partners-popup', function(e) {
        e.stopPropagation();
console.log('partners');
        if ( $(this).closest('form').hasClass('mobile_ppi') ) {
                is_mobile = true;
        }
        TogglePopup($('#PartnersPopup'), true, is_mobile);
});
